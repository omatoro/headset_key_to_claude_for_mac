...
protocol NowPlayingInterceptorDelegate: AnyObject {
    // sourceCommand: MPRemoteCommandCenterのどのコマンドとして届いたか
    // (togglePlayPause / play / pause / nextTrack 等)。ヘッドセット由来の
    // コマンド種別を実測で識別するために引数として渡す。
...
class NowPlayingInterceptor {
    ...
    /// Claude Desktopが最前面の間だけNow Playingをクレームする。
    /// falseの間はクレームを解除してrcdのルーティングをシステム本来の状態へ返し、
    /// ヘッドセットのボタンがYouTube等へ届く「素通し」を実現する。
    private(set) var isClaimActive = true
    ...
    private func refreshTick() {
        ...
        guard isClaimActive else { return }
        reassertNowPlaying()
    }

    /// 最前面アプリの変化に応じてクレームを切り替える(AppDelegateから呼ばれる)。
    /// OFF時はコマンド受付自体も無効化する。クレーム解除だけではrcdのルーティングが
    /// 本アプリに残り、ヘッドセットのボタンがどこにも効かなくなる実測があったため
    /// (isEnabled=falseでrcdへ「受けない」を通知し、次のNow Playing候補へ譲る)。
    func setClaimActive(_ active: Bool) {
        guard isClaimActive != active else { return }
        isClaimActive = active
        registeredCommands.forEach { $0.isEnabled = active }
        let infoCenter = MPNowPlayingInfoCenter.default()
        if active {
            reassertNowPlaying()
            debugLog("NowPlaying claim ON (Claude Desktop frontmost)")
        } else {
            infoCenter.nowPlayingInfo = nil
            infoCenter.playbackState = .stopped
            debugLog("NowPlaying claim OFF — commands disabled, released to system routing")
        }
    }
    ...
    func reassertNowPlaying() {
        ...
        infoCenter.nowPlayingInfo = [
            MPMediaItemPropertyTitle: "headset_key_to_claude_for_mac",
    ...
    private func handleCommand(_ key: MediaKey, sourceCommand: String) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            self.delegate?.nowPlayingInterceptor(self, receivedKey: key, sourceCommand: sourceCommand)
        }
    }
